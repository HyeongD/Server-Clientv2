...
public class EchoServer extends AbstractServer {
    ...
    public void handleMessageFromClient(Object msg, ConnectionToClient client) {
        ...
        // client.setInfo("login", command.substring(3));
        ...
        if (command.startsWith("#login")) {
            if (client.getInfo("loginID") == null) {
                try {
                    ...
                    // serverUI.display(client.getInfo("loginID").toString());
            ...
            if (client.getInfo("loginID") == null) {
                serverUI.display("Unknnown client did not login. " + "Terminating connnection.");
                try {
    ...
    public void handleMessageFromServerUI(String message) {
        ...
        if (message.startsWith("#stop")) {
            if (isListening()) {
                ...
                serverUI.display("Cannot stop the servert before it is restarted.");
            }
            return;
        }
        ...
        if (message.startsWith("#setport")) {
            if ((getNumberOfClients() != 0) || (isListening())) {
                serverUI.display("Cannot change port while clients " + "are connected or while server is listenning");
            } else {
                try {
                    int port = 0;
